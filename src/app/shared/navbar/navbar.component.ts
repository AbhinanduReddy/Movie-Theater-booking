...
export class NavbarComponent implements OnInit {
    ...
    constructor( public location: Location, private element : ElementRef, public auth: AuthenticationService, 
        private http: HttpClient, private theater: TheaterService,
        private confirmServc: ConfirmationService,
  ...
  confirmOperation(ticketId: number) {
    this.confirmServc.confirm({
      message: 'Are you sure you want to perform this action?',
      header: 'Confirmation',
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        // Action to perform when confirmed
        this.cancelBooking(ticketId);
      },
      reject: () => {
        // Action to perform when not confirmed
      }
    });
  }
    ...
    updateUserRewards(): void {
        // Assuming the user's ID is stored in auth service
        const userId = this.auth.user?.user_id;
        if (userId) {
            this.http.get(`/api/user/rewards/${userId}`).subscribe(
                (res: any) => {
                    console.log('Rewards updated:', res);
