...
selectShowTime(st:any){
 ...
 ngOnDestroy(){
  ...
  /*isPastTime(showTime: any): boolean {
    const currentTime = new Date();
    const showDateTime = new Date(this.selectedDate);
    // const [hours, minutes] = showTime.show_time.split(':').map(Number);
    // const timeParts = showTime.show_time.split(':');
    // showDateTime.setHours(timeParts[0], timeParts[1], 0);

  const [hours, minutes] = showTime.show_time.split(':').map(Number);
  showDateTime.setHours(hours, minutes, 0, 0);

  console.log(`Current Time: ${currentTime}`);
  console.log(`Show Time: ${showDateTime} (${showTime.show_time})`);
  console.log(`Is Past: ${showDateTime < currentTime}`);
